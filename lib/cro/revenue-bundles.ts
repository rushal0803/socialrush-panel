...
export function revenueBundlesForPlatform(platform: SmmPlatformId) {
  return BUNDLES.filter((bundle) => bundle.platform === platform);
}

export function resolveRevenueBundle(bundle: RevenueBundle, services: SmmService[]) {
  const available = bundle.serviceCodes
    .map((code) => services.find((service) => service.code === code))
    .filter((service): service is SmmService => Boolean(service && service.active && service.acceptingOrders && !service.paused && service.pricePer1000 > 0));

  const quantityFor = (service: SmmService) => {
    const floor = Math.max(service.minQuantity, service.step);
    const ceiling = Math.min(service.maxQuantity, bundle.recommendedQuantity);
    if (ceiling < floor) return floor;
    return Math.max(floor, Math.floor(ceiling / service.step) * service.step);
  };

  const items = available.map((service) => {
    const quantity = quantityFor(service);
    return { service, quantity, total: (service.pricePer1000 * quantity) / 1000 };
  });
