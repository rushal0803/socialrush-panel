...
export async function getLiveServiceFacts(platform: string, serviceName: string): Promise<LiveServiceFacts | null> {
  ...
  try {
    ...
    const { data } = await db.from("services").select("id,rate,min,max,delivery_time,refill_policy,is_active,status,health_status,accepts_new_orders,important_instruction")
  ...
  return {
    ...
    deliveryTime: catalogService.deliveryTime, refillPolicy: catalogService.refillPolicy, available: catalogService.isActive, healthStatus: "catalog", importantInstruction: catalogService.importantInstruction,
  };
}
