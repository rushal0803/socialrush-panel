...
export default function AdminPendingPaymentVerification({ orderId, paymentStatus, customerNote, amount }: Props) {
  ...
  async function confirmPayment() {
    ...
    try {
      const response = await fetch(`/api/admin/orders/${orderId}/status`, {
        ...
        body: JSON.stringify({ status: "processing", note: "Manual payment receipt verified against the submitted UTR." }),
      });
  ...
  return (
    <section className="rounded-3xl border border-amber-300 bg-amber-50 p-5 shadow-sm sm:p-6">
      <p className="text-[10px] font-black uppercase tracking-[.16em] text-amber-700">Action required · Manual payment verification</p>
      <h2 className="mt-2 text-lg font-black text-slate-950">Verify ₹{Number(amount).toLocaleString("en-IN")} before fulfillment</h2>
      <p className="mt-2 text-sm leading-6 text-slate-700">Check the actual incoming payment in your UPI or bank account. Match the exact amount and submitted UTR below. Do not approve based on a screenshot alone.</p>
      <div className="mt-4 rounded-2xl border border-amber-200 bg-white p-4 text-sm text-slate-800">
        ...
        <p className="mt-2 break-words font-semibold">{customerNote || "Payment details were not saved."}</p>
      </div>
