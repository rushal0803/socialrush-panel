...
export default function ManualUpiAddFunds({ inrPerUsd }: { inrPerUsd: number | null }) {
  ...
  useEffect(() => {
    if (!paymentStarted || paymentMethod !== "upi") {
    ...
    const handleReturn = () => {
      ...
      window.setTimeout(() => utrInputRef.current?.focus(), 220);
    };
  ...
  async function copyAmount() { await navigator.clipboard.writeText(amount.toFixed(2)).catch(() => undefined); setCopiedAmount(true); setTimeout(() => setCopiedAmount(false), 1500); }
  ...
  return <main className="mx-auto max-w-3xl px-4 py-6 pb-28 sm:py-10">
    <div className="mb-6"><p className="text-xs font-black uppercase tracking-[.2em] text-orange-400">Wallet</p><h1 className="mt-2 text-3xl font-black text-white">Add funds</h1><p className="mt-2 text-sm text-slate-400">UPI is the fastest option. Pay from any compatible UPI app, then paste the UTR so we can verify and credit your wallet safely.</p></div>
    ...
    <section className="rounded-3xl border border-orange-400/20 bg-[#11141c] p-5 shadow-2xl sm:p-7">
      ...
      <div className="mt-3 flex items-center rounded-2xl border border-orange-400/30 bg-black/20 px-4"><span className="text-xl font-black text-orange-300">₹</span><input inputMode="decimal" value={amountText} disabled={paymentStarted} onChange={(e)=>setAmountText(e.target.value.replace(/[^0-9.]/g,""))} className="w-full bg-transparent px-3 py-4 text-2xl font-black text-white outline-none disabled:cursor-not-allowed disabled:opacity-60" aria-label="Amount" /></div>
      <p className="mt-2 text-xs text-slate-500">Minimum ₹100 · Maximum ₹5,00,000</p>
      <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-5">{quickAmounts.map((value)=><button key={value} disabled={paymentStarted} onClick={()=>setAmountText(String(value))} className="rounded-xl border border-white/10 bg-white/[.03] px-2 py-3 text-xs font-bold text-slate-300 hover:border-orange-400/40 disabled:cursor-not-allowed disabled:opacity-40">₹{value.toLocaleString("en-IN")}</button>)}</div>
      {!paymentStarted ? <>
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <button type="button" onClick={()=>{setPaymentMethod("upi");setError("");}} className={`rounded-2xl border p-4 text-left sm:col-span-2 ${paymentMethod==="upi"?"border-orange-400 bg-orange-500/10":"border-white/10 bg-black/20"}`}><div className="flex items-center justify-between"><strong className="text-sm text-white">UPI · All Apps</strong><span className="rounded-full bg-emerald-500/10 px-2 py-1 text-[10px] font-black text-emerald-300">Fastest</span></div><p className="mt-2 text-xs text-slate-400">Google Pay, PhonePe, Paytm, BHIM & other compatible UPI apps</p></button>
        ...
        {paymentMethod==="upi" ? <>
          ...
          <div className="mt-5 rounded-2xl border border-orange-400/20 bg-black/20 p-4 sm:p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="text-[10px] font-black uppercase tracking-[.18em] text-orange-300">Universal UPI</p><h2 className="mt-1 text-lg font-black text-white">Pay in one tap with any UPI app</h2><p className="mt-2 text-xs leading-5 text-slate-400">The standard UPI link opens compatible installed apps with your amount already prepared.</p></div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <div className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[.025] p-3">
              ...
              <div className="flex items-center justify-between gap-3 rounded-xl border border-orange-400/20 bg-orange-500/[.06] p-3">
                <div><p className="text-[10px] font-black uppercase tracking-wider text-slate-500">Exact amount</p><p className="mt-1 text-base font-black text-orange-200">₹{validAmount ? amount.toLocaleString("en-IN",{minimumFractionDigits:2}) : "0.00"}</p></div>
                <button type="button" disabled={!validAmount} onClick={()=>void copyAmount()} className="shrink-0 rounded-xl border border-orange-400/20 px-3 py-2 text-xs font-bold text-orange-300 disabled:opacity-40">{copiedAmount?"Copied":"Copy"}</button>
              </div>
            </div>
          </div>
          <a href={upiHref} onClick={(e)=>{if(!validAmount||!upiConfigured){e.preventDefault();setError(!upiConfigured?"UPI is temporarily unavailable.":"Enter a valid amount.");return;} setError("");setPaymentStarted(true);}} className={`mt-4 flex w-full items-center justify-center rounded-xl px-5 py-4 text-sm font-black ${validAmount&&upiConfigured?"bg-gradient-to-r from-orange-500 to-amber-400 text-black":"cursor-not-allowed bg-white/10 text-slate-500"}`}>Pay Now · Open UPI Apps</a>
          <button type="button" disabled={!validAmount||!upiConfigured} onClick={()=>{setError("");setPaymentStarted(true);}} className="mt-3 w-full rounded-xl border border-orange-400/25 bg-orange-500/[.06] px-5 py-3.5 text-sm font-black text-orange-200 disabled:opacity-50">I already paid · Enter UTR</button>
          <p className="mt-3 text-center text-[11px] leading-5 text-slate-500">If the app chooser does not open, use Copy UPI ID + Copy Amount and pay from any UPI app. On desktop, use those details on your phone.</p>
        ...
        <p className="text-xs font-black uppercase tracking-[.18em] text-orange-300">Final step</p><h2 className="mt-2 text-xl font-black text-white">Paste your UTR and submit</h2><p className="mt-2 text-xs leading-5 text-slate-400">{paymentMethod === "usdt_trc20" ? "Paste the TRC20 transaction hash / TxID from your successful USDT transfer." : paymentMethod === "bank_transfer" ? "Enter the UTR / Transaction ID from your successful bank transfer." : "Enter the UTR / Transaction ID shown in your UPI app."} This helps us match your payment safely.</p>
        ...
        <label className="mt-4 block text-[11px] font-black uppercase tracking-wider text-slate-400">{paymentMethod === "usdt_trc20" ? "TRC20 TxID" : "UTR / Transaction ID"}</label><div className="mt-2 flex gap-2"><input ref={utrInputRef} value={utr} onChange={(e)=>setUtr(e.target.value.slice(0,80))} placeholder={paymentMethod === "usdt_trc20" ? "Paste TRC20 transaction hash" : "Enter UTR / Transaction ID"} className="min-h-12 min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-4 text-sm text-white outline-none focus:border-orange-400" /><button type="button" onClick={()=>void pasteTransactionId()} className="min-h-12 shrink-0 rounded-xl border border-orange-400/25 bg-orange-500/[.08] px-4 text-xs font-black text-orange-200">Paste</button></div>
        {error && <p className="mt-3 rounded-xl bg-red-500/10 p-3 text-xs text-red-300">{error}</p>}
        <button disabled={submitting} onClick={submitPayment} className="mt-4 w-full rounded-xl bg-gradient-to-r from-orange-500 to-amber-400 px-5 py-4 text-sm font-black text-black disabled:opacity-60">{submitting?"Submitting…":paymentMethod==="bank_transfer"?"Submit for Verification":paymentMethod==="upi"?"Submit UTR & Add Funds":"Confirm & Add Funds"}</button>
