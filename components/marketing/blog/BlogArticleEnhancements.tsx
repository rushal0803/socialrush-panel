...
export default function BlogArticleEnhancements({ toc, articleUrl, showProgress, mobileToc = true, desktopToc = true, showShare = true }: { toc: TocItem[]; articleUrl: string; showProgress: boolean; mobileToc?: boolean; desktopToc?: boolean; showShare?: boolean }) {
  ...
  const links = <ol className="grid gap-2">{toc.map((item) => <li key={item.id}><a href={`#${item.id}`} aria-current={activeId === item.id ? "location" : undefined} className={`block rounded-lg px-2 py-2 text-sm font-semibold transition ${activeId === item.id ? "bg-orange-400/15 text-orange-200" : "text-slate-300 hover:bg-white/[.06] hover:text-white"}`}>{item.label}</a></li>)}</ol>;
  return <>
    ...
    {toc.length && mobileToc ? <div className="mt-6 lg:hidden"><button type="button" aria-expanded={open} aria-controls="mobile-toc" onClick={() => setOpen(!open)} className="flex min-h-11 w-full items-center justify-between rounded-xl border border-white/10 bg-white/[.04] px-4 text-left font-extrabold text-white">On this page <span aria-hidden="true">{open ? "−" : "+"}</span></button>{open ? <nav id="mobile-toc" aria-label="Table of contents" className="mt-2 rounded-xl border border-white/10 bg-[#101520] p-3">{links}</nav> : null}</div> : null}
    {toc.length && desktopToc ? <aside className="sticky top-28 hidden self-start rounded-2xl border border-white/10 bg-[#101520] p-4 shadow-[0_12px_28px_rgba(0,0,0,.2)] lg:block"><h2 className="mb-3 text-sm font-extrabold text-white">On this page</h2><nav aria-label="Table of contents">{links}</nav></aside> : null}
