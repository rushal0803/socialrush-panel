  ...
  return <PublicShell>
    ...
    <main>
      <section className="relative overflow-hidden border-b border-white/10 bg-[#0b0d13] px-5 py-16 sm:px-6 lg:px-8 lg:py-24"><div className="pointer-events-none absolute inset-x-0 top-0 h-64 bg-[radial-gradient(ellipse_at_12%_0%,rgba(255,118,0,.18),transparent_64%)]"/><div className="relative mx-auto grid max-w-6xl gap-7 lg:grid-cols-[1.2fr_.8fr] lg:items-end">
        ...
        <aside className="sr-surface rounded-2xl p-5"><p className="text-xs font-black uppercase tracking-[.14em] text-orange-200">Market information</p><p className="mt-3 text-sm leading-7 text-slate-300">{market.paymentNote}</p><p className="mt-3 border-t border-white/10 pt-3 text-xs leading-5 text-slate-400">Display currency helps planning; checkout remains INR-authoritative.</p></aside>
      </div></section>
      ...
      <section className="border-y border-white/10 bg-[#0b0d13] px-5 py-16 sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl"><h2 className="text-3xl font-black">Supported platforms</h2><p className="mt-3 text-sm leading-7 text-slate-300">Explore current service availability by platform. Service pages remain the source of truth for specific options.</p><div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">{platforms.map(([platform,key]) => <Link key={platform} href={`/services?platform=${key}`} className="rounded-xl border border-white/10 bg-white/[.03] p-4 text-center text-sm font-bold text-white transition hover:border-orange-400/40 hover:text-orange-200">{platform}</Link>)}</div></div></section>
      {services.length ? <section id="market-services" className="px-5 py-16 sm:px-6 lg:px-8"><div className="mx-auto max-w-6xl"><h2 className="text-3xl font-black">Popular services in {market.name}</h2><p className="mt-3 text-sm leading-7 text-slate-300">These country-specific pages use the current catalog and the established ordering flow.</p><div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{services.map((service) => <Link key={service.serviceSlug} href={`/${market.slug}/${service.serviceSlug}`} className="rounded-2xl border border-white/10 bg-[#101219] p-5 font-black transition hover:border-orange-400/40">{service.h1}</Link>)}</div></div></section> : null}
