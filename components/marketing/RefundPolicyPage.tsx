...
export default function RefundPolicyPage({
  ...
  return (
    <PublicShell tone="light3d">
      <div className="relative overflow-x-clip bg-[radial-gradient(circle_at_16%_16%,rgba(121,177,255,.2),transparent_38%),radial-gradient(circle_at_84%_10%,rgba(255,147,214,.2),transparent_40%),radial-gradient(circle_at_52%_86%,rgba(111,224,255,.16),transparent_44%),linear-gradient(180deg,#f8fbff_0%,#eef4ff_100%)] pb-20 pt-8 sm:pb-24 lg:pt-10">
        <div className="pointer-events-none absolute inset-0">
          <motion.div
            aria-hidden
            animate={{ y: [0, -14, 0], x: [0, 8, 0] }}
            transition={{ duration: 11, repeat: Infinity, ease: "easeInOut" }}
            className="absolute -left-20 top-10 h-72 w-72 rounded-full bg-pink-200/40 blur-3xl"
          />
          <motion.div
            aria-hidden
            animate={{ y: [0, 10, 0], x: [0, -8, 0] }}
            transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
            className="absolute right-[-5rem] top-14 h-80 w-80 rounded-full bg-cyan-200/40 blur-3xl"
          />
          <motion.div
            aria-hidden
            animate={{ y: [0, -9, 0] }}
            transition={{ duration: 9, repeat: Infinity, ease: "easeInOut" }}
            className="absolute bottom-8 left-1/3 h-64 w-64 rounded-full bg-violet-200/35 blur-3xl"
          />
        </div>
          ...
          <div className="mx-auto max-w-7xl">
            ...
            >
              ...
              <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px] lg:items-start">
                ...
                >
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#5b76aa]">Refund flow highlights</p>
                  <p className="mt-3 text-lg font-black text-[#17366f]">Policy confidence for every campaign</p>
                  <div className="mt-4 space-y-3">
                    {heroPoints.map((item) => {
                      const Icon = item.icon;
                      return (
                        <div key={item.title} className="flex items-center gap-3 rounded-xl border border-[#dce7ff] bg-white/80 px-3 py-2">
                          <span className="grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-r from-[#ff67b2] via-[#8b8dff] to-[#46c3ff] text-white">
                            <Icon className="h-4 w-4" />
                          </span>
                          <p className="text-xs font-bold text-[#35548d]">{item.title}</p>
                        </div>
                      );
                    })}
                  </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {trustCards.map((card, index) => {
                ...
                return (
                  ...
                  >
                    <span className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-[#ff67b2] via-[#8b8dff] to-[#46c3ff] text-white shadow-[0_10px_20px_-12px_rgba(117,109,255,.7)]">
                      <Icon className="h-4 w-4" />
                    </span>
                    <p className="mt-3 text-xs font-black text-[#1f3d77]">{card.title}</p>
        ...
        <section className="relative px-4 pt-8 sm:px-6 lg:px-8">
          <div className="mx-auto flex max-w-7xl flex-col items-stretch gap-3 rounded-3xl border border-white/80 bg-white/75 p-5 shadow-[0_20px_44px_-30px_rgba(15,23,42,.36)] backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between sm:p-6">
            <div>
              <p className="text-sm font-black text-[#1f3d77]">Need help with a refund or wallet review?</p>
              <p className="mt-1 text-xs text-[#6079ab]">Our support team can review payment records, campaign status, and account details.</p>
            </div>
