    ...
    <PublicShell tone="light3d">
      ...
      <section className="relative overflow-hidden px-4 pb-16 pt-14 sm:px-6 sm:pb-20 sm:pt-20 lg:px-8 lg:pb-24">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1.18fr_.82fr]">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-white/90 bg-white/75 px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-[#5270aa] shadow-sm backdrop-blur-xl">
            ...
            <p className="mt-6 max-w-2xl text-base leading-8 text-[#526d9f] sm:text-lg">{page.intro}</p>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-[#6079a7]">{page.overview}</p>

            <div className="mt-7 flex flex-wrap gap-2">
              {trustItems.map((item) => (
                <span key={item} className="inline-flex items-center gap-1.5 rounded-full border border-[#dce7ff] bg-white/75 px-3 py-2 text-[11px] font-bold text-[#35548d] shadow-sm">
            ...
            <div className="mt-8 flex flex-col gap-3 min-[420px]:flex-row">
              <Link href={packagesHref} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#ff67b2] via-[#8b8dff] to-[#46c3ff] px-6 py-3 text-sm font-black text-white shadow-[0_14px_30px_-14px_rgba(117,109,255,.65)] transition hover:-translate-y-0.5">
          ...
          <aside className="rounded-[2rem] border border-white/90 bg-white/78 p-5 shadow-[0_30px_70px_-35px_rgba(35,60,120,.5)] backdrop-blur-2xl sm:p-7">
            ...
            <p className="mt-6 text-[10px] font-black uppercase tracking-[0.15em] text-[#7890bb]">Current catalog rate</p>
            {formattedPrice ? (
              <p className="mt-2 text-3xl font-black text-[#17366f]">
                {formattedPrice} <span className="text-sm text-[#6079a7]">per 1K</span>
              </p>
            ) : (
              <p className="mt-2 text-2xl font-black text-[#17366f]">View latest price</p>
            )}
            ...
            <dl className="mt-6 grid gap-3 border-t border-[#dce7ff] pt-5 text-xs">
              <div className="flex items-center justify-between gap-4">
                <dt className="text-[#7890bb]">Delivery estimate</dt>
                <dd className="text-right font-black text-[#294981]">{page.service.deliveryTime}</dd>
              </div>
              <div className="flex items-center justify-between gap-4">
                <dt className="text-[#7890bb]">Refill support</dt>
                <dd className="text-right font-black text-[#294981]">{page.service.refillPolicy}</dd>
              </div>
              <div className="flex items-center justify-between gap-4">
                <dt className="text-[#7890bb]">Required</dt>
                <dd className="max-w-[60%] text-right font-black text-[#294981]">{page.destination}</dd>
              </div>
            </dl>
            <Link href={packagesHref} className="mt-6 inline-flex min-h-12 w-full items-center justify-center rounded-xl border border-[#d4e1ff] bg-[#f7faff] px-5 py-3 text-sm font-black text-[#35548d] transition hover:border-[#9fb8ee] hover:bg-white">
      ...
      <section className="bg-white/65 px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-2xl">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Campaign benefits</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight text-[#14316a]">A clear way to support your {page.platform.label} presence</h2>
          </div>
          <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {page.benefits.map((benefit) => (
              <article key={benefit.title} className="rounded-3xl border border-[#dce7ff] bg-white p-5 shadow-[0_18px_42px_-30px_rgba(28,54,108,.45)] sm:p-6">
                <CheckCircle2 className="h-6 w-6 text-emerald-600" />
                <h3 className="mt-4 text-base font-black text-[#17366f]">{benefit.title}</h3>
                <p className="mt-2 text-sm leading-6 text-[#6079a7]">{benefit.description}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
          <article className="rounded-[2rem] border border-white/90 bg-white/78 p-6 shadow-[0_26px_60px_-38px_rgba(35,60,120,.5)] backdrop-blur-2xl sm:p-8">
            <span className="grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-[#f6eaff] to-[#e7f7ff] text-[#6873d4]">
              <Users className="h-6 w-6" />
            </span>
            <p className="mt-6 text-xs font-black uppercase tracking-[0.16em] text-blue-600">Who this service is for</p>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {audiences.map((audience) => (
                <div key={audience} className="flex items-start gap-3 rounded-2xl border border-[#e1eaff] bg-white/85 p-4 text-sm font-bold leading-6 text-[#466293]">
          ...
          <article className="rounded-[2rem] border border-white/90 bg-[linear-gradient(145deg,#122b5d,#2d3f87_55%,#176f9d)] p-6 text-white shadow-[0_30px_70px_-38px_rgba(28,47,105,.72)] sm:p-8">
            <span className="grid h-12 w-12 place-items-center rounded-2xl border border-white/15 bg-white/10 text-cyan-200">
              <BarChart3 className="h-6 w-6" />
            </span>
            <p className="mt-6 text-xs font-black uppercase tracking-[0.16em] text-cyan-200">How delivery works</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight">A visible process, not a mystery order</h2>
            <p className="mt-5 text-sm leading-8 text-blue-100/80">{deliveryExplanation}</p>
      ...
      <section className="bg-white/55 px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Before you order</p>
              <h2 className="mt-3 text-3xl font-black tracking-tight text-[#14316a]">
          ...
          <div className="mt-10 rounded-[2rem] border border-white/90 bg-white/80 p-6 shadow-[0_24px_58px_-38px_rgba(35,60,120,.5)] backdrop-blur-xl sm:p-8">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-amber-100 to-pink-100 text-amber-700">
                <Lightbulb className="h-5 w-5" />
              </span>
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[#6e79b2]">{page.platform.label} growth tips</p>
                <h2 className="mt-1 text-2xl font-black text-[#14316a]">Make new attention more useful</h2>
              </div>
            </div>
            <div className="mt-6 grid gap-3 md:grid-cols-3">
              {growthTips.map((tip, index) => (
                <article key={tip} className="rounded-2xl border border-[#e1eaff] bg-[#f8fbff] p-5">
                  <span className="text-2xl font-black text-blue-100">0{index + 1}</span>
                  <h3 className="mt-2 text-sm font-black leading-6 text-[#294981]">{tip}</h3>
                </article>
              ))}
            </div>
          </div>
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Four simple steps</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight text-[#14316a]">How it works</h2>
            <p className="mt-4 text-sm leading-7 text-[#6079a7]">Review every important campaign detail before you confirm your order.</p>
          </div>
          <ol className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {steps.map((step, index) => {
              const Icon = step.icon;
              return (
                <li key={step.title} className="relative rounded-3xl border border-white/90 bg-white/80 p-6 shadow-[0_20px_48px_-32px_rgba(35,60,120,.5)]">
                  <span className="absolute right-5 top-5 text-3xl font-black text-blue-100">0{index + 1}</span>
                  <span className="grid h-12 w-12 place-items-center rounded-2xl bg-blue-50 text-blue-600">
                    <Icon className="h-6 w-6" />
                  </span>
                  <h3 className="mt-5 text-base font-black text-[#17366f]">{step.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-[#6079a7]">{step.description}</p>
                </li>
              );
            })}
          </ol>
      ...
      <section className="bg-[#0b1d43] px-4 py-16 text-white sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-2xl">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-300">Why choose SocialRUSH</p>
            ...
            {reasons.map((reason) => {
              ...
              return (
                <article key={reason.title} className="rounded-3xl border border-white/10 bg-white/[0.06] p-5 backdrop-blur sm:p-6">
                  ...
                  <p className="mt-2 text-sm leading-6 text-blue-100/65">{reason.description}</p>
                </article>
              );
            })}
          </div>
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Related services</p>
              <h2 className="mt-3 text-3xl font-black tracking-tight text-[#14316a]">Explore another growth campaign</h2>
            </div>
            <Link href="/services" className="text-sm font-black text-blue-600 hover:text-blue-700">View all services →</Link>
          </div>
            ...
            {page.related.map((relatedSlug) => {
              ...
              return (
                <Link key={related.slug} href={relatedHref} className="group rounded-3xl border border-white/90 bg-white/80 p-5 shadow-[0_18px_42px_-30px_rgba(28,54,108,.45)] transition hover:-translate-y-1 hover:border-[#cbdcff] sm:p-6">
                  ...
                  <h3 className="mt-4 text-base font-black text-[#17366f]">{related.displayName}</h3>
                  <p className="mt-2 text-sm leading-6 text-[#6079a7]">{related.intro}</p>
      ...
      <section className="bg-white/65 px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.7fr_1.3fr]">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Helpful answers</p>
            <h2 className="mt-3 text-3xl font-black tracking-tight text-[#14316a]">Frequently asked questions</h2>
            <p className="mt-4 text-sm leading-7 text-[#6079a7]">Review pricing, delivery and ordering details before starting your campaign.</p>
          </div>
          <div className="space-y-3">
            {faqs.map((faq, index) => (
              <details key={faq.question} className="group rounded-2xl border border-[#dce7ff] bg-white p-5 shadow-sm" open={index === 0}>
                <summary className="cursor-pointer list-none pr-8 marker:hidden">
                  <h3 className="inline text-sm font-black text-[#17366f]">{faq.question}</h3>
                  <span className="float-right text-lg font-bold text-blue-600 transition group-open:rotate-45">+</span>
                </summary>
                <p className="mt-3 border-t border-[#e7eeff] pt-3 text-sm leading-7 text-[#6079a7]">{faq.answer}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8 lg:py-20">
        <div className="mx-auto max-w-5xl rounded-[2rem] bg-gradient-to-br from-[#17366f] via-[#3455a5] to-[#178ac2] p-7 text-center text-white shadow-[0_30px_70px_-35px_rgba(35,60,120,.7)] sm:p-10">
          <h2 className="text-3xl font-black tracking-tight">Ready to start your {page.displayName} campaign?</h2>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-blue-50/80">Compare available packages, review the latest rate and confirm every detail before placing your order.</p>
          <div className="mt-7 flex flex-col justify-center gap-3 min-[420px]:flex-row">
            <Link href={packagesHref} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-white px-6 py-3 text-sm font-black text-[#17366f] transition hover:-translate-y-0.5">
              View Packages
            </Link>
