...
export default function CrossSellRecommendations({ serviceCode, compact = false }: { serviceCode: string; compact?: boolean }) {
  ...
  const recommendations = service ? activeSmmServices.filter((item) => item.platform === service.platform && item.code !== service.code).slice(0, 2) : [];
  ...
  return <section className={`rounded-2xl border border-orange-400/20 bg-orange-500/[.06] ${compact ? "p-4" : "p-5 sm:p-6"}`}>
    <p className="text-[10px] font-black uppercase tracking-[.15em] text-orange-200">Complete your {service.platform} plan</p>
    <h2 className="mt-2 text-lg font-black text-white">Complementary services, when they fit your next goal</h2>
    ...
    <div className="mt-4 grid gap-2 sm:grid-cols-2">{recommendations.map((item) => <Link key={item.code} href={`/order-summary?service=${item.code}`} onClick={() => track("cross_sell_click", { source_service: service.code, recommended_service: item.code })} className="group flex min-h-12 items-center justify-between rounded-xl border border-white/10 bg-black/20 px-4 text-sm font-bold text-white transition hover:border-orange-400/50 hover:bg-orange-500/10"><span>{item.name}</span><ArrowRight className="h-4 w-4 text-orange-300 transition group-hover:translate-x-0.5" /></Link>)}</div>
  </section>;
}
