...
const features: { icon: IconName; title: string; text: string; tone: string }[] = [
  { icon: "campaign", title: "Smart Campaign Management", text: "Launch and manage social growth campaigns from a single professional dashboard.", tone: "bg-blue-500/15 text-blue-300" },
  { icon: "tracking", title: "Real-Time Order Tracking", text: "Track campaign progress, delivery status, and performance updates instantly.", tone: "bg-cyan-500/15 text-cyan-300" },
  ...
  { icon: "quality", title: "Premium Quality Delivery", text: "Professionally managed campaigns focused on consistency and reliability.", tone: "bg-violet-500/15 text-violet-300" },
...
export default function WhySocialRush() {
  return <section id="why" className="relative overflow-hidden bg-[#06142f] px-5 py-20 text-white sm:px-6 lg:px-8 lg:py-28">
    <motion.div aria-hidden className="absolute -left-32 top-24 h-96 w-96 rounded-full bg-blue-600/20 blur-3xl" animate={{ x: [0, 35, 0], y: [0, -24, 0] }} transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}/>
    <motion.div aria-hidden className="absolute -right-32 bottom-20 h-96 w-96 rounded-full bg-cyan-500/15 blur-3xl" animate={{ x: [0, -30, 0], y: [0, 30, 0] }} transition={{ duration: 14, repeat: Infinity, ease: "easeInOut" }}/>
    <div className="hero-grid absolute inset-0 opacity-[.08]"/>
    <div className="relative mx-auto max-w-7xl">
      <motion.div initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mx-auto max-w-3xl text-center"><p className="text-xs font-bold uppercase tracking-[.22em] text-blue-400">A complete growth operating system</p><h2 className="mt-5 text-3xl font-bold tracking-[-.04em] sm:text-4xl lg:text-5xl">Why 10,000+ Businesses Choose SocialRUSH</h2><p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-slate-400 sm:text-base">SocialRUSH combines growth technology, campaign management, analytics, and professional support into one powerful platform.</p></motion.div>

      <div className="mt-14 grid items-center gap-10 xl:grid-cols-[1.05fr_.95fr]">
        <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: .65 }} className="relative"><motion.div className="absolute -left-5 top-20 z-20 hidden rounded-2xl border border-white/10 bg-white/10 p-3 shadow-2xl backdrop-blur-xl sm:flex sm:items-center sm:gap-3" animate={{ y: [0, -9, 0] }} transition={{ duration: 5, repeat: Infinity }}><span className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-400/15 text-emerald-300">↗</span><div><p className="text-[9px] text-slate-400">Campaign velocity</p><p className="text-xs font-bold">+31.8% this month</p></div></motion.div><div className="rounded-[30px] border border-white/10 bg-white/[.065] p-3 shadow-[0_35px_90px_-30px_rgba(0,0,0,.65)] backdrop-blur-xl"><div className="overflow-hidden rounded-[23px] border border-white/10 bg-[#0a1c3e]"><div className="flex h-13 items-center justify-between border-b border-white/10 bg-white/[.04] px-5 py-3"><div><p className="text-[8px] font-bold uppercase tracking-[.18em] text-blue-400">Growth intelligence</p><p className="mt-1 text-xs font-bold">Live campaign dashboard</p></div><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-emerald-400"/><span className="text-[8px] font-bold text-emerald-300">Live</span></div></div><div className="p-4 sm:p-6"><div className="grid grid-cols-3 gap-3">{[["Instagram","+28.4%","Audience growth"],["YouTube","+19.7%","Video performance"],["Facebook","+34.2%","Engagement rate"]].map(([platform,value,label],index) => <motion.div key={platform} whileHover={{ y: -4 }} className="rounded-2xl border border-white/10 bg-white/[.055] p-3 sm:p-4"><div className="flex items-center justify-between"><span className={`h-2 w-2 rounded-full ${["bg-fuchsia-400","bg-red-400","bg-blue-400"][index]}`}/><span className="text-[8px] font-bold text-emerald-300">{value}</span></div><p className="mt-4 text-[9px] text-slate-400">{platform}</p><p className="mt-1 text-[9px] font-bold sm:text-xs">{label}</p></motion.div>)}</div><div className="mt-4 rounded-2xl border border-white/10 bg-white/[.045] p-4"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold">Multi-platform performance</p><p className="mt-1 text-[8px] text-slate-500">Last 30 days</p></div><span className="rounded-full bg-blue-500/15 px-2.5 py-1 text-[8px] font-bold text-blue-300">+26.9% overall</span></div><div className="mt-6 flex h-32 items-end gap-2">{[34,48,40,62,54,76,68,85,72,94,82,100,88,96].map((height,index) => <motion.span key={index} initial={{ height: 0 }} whileInView={{ height: `${height}%` }} viewport={{ once: true }} transition={{ duration: .7, delay: index * .035 }} className="flex-1 rounded-t bg-gradient-to-t from-blue-600 to-cyan-400"/>)}</div></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{[["Instagram Authority Campaign","Processing","72%"],["YouTube Video Promotion","Completed","100%"],["Facebook Brand Reach","Delivering","48%"],["X Profile Growth","Scheduled","12%"]].map(([name,status,progress]) => <div key={name} className="rounded-xl border border-white/10 bg-white/[.035] p-3"><div className="flex items-center justify-between gap-2"><p className="truncate text-[8px] font-semibold">{name}</p><span className={`h-2 w-2 rounded-full ${status === "Completed" ? "bg-emerald-400" : status === "Scheduled" ? "bg-amber-400" : "bg-blue-400"}`}/></div><div className="mt-3 h-1 overflow-hidden rounded-full bg-white/10"><motion.div initial={{ width: 0 }} whileInView={{ width: progress }} viewport={{ once: true }} transition={{ duration: 1, delay: .3 }} className="h-full rounded-full bg-gradient-to-r from-blue-500 to-cyan-400"/></div><div className="mt-2 flex justify-between text-[7px] text-slate-500"><span>{status}</span><span>{progress}</span></div></div>)}</div></div></div></div></motion.div>
