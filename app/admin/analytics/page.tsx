...
export default async function Page({ searchParams }: { searchParams: { range?: string } }) {
  ...
  const cards = [["Unique sessions or users", visitors], ["Order starts", starts], ["Orders created", created], ["Order conversion", pct(starts, created)], ["Verified payments", count("payment_completed")], ["Wallet top-ups", count("wallet_topup_completed")], ["Support tickets", supportCount], ["Repeat-order rate", pct(completedOrders.length, repeat)]];
  ...
  return <main className="p-4 pb-24 sm:p-8"><div className="mx-auto max-w-[1500px]">
    ...
    <p className="mt-2 text-sm text-slate-400">First-party funnel data and verified business records. Counts are raw events except the unique audience card. Reporting timezone: Asia/Kolkata. Collection started: {first ? new Date(first).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" }) : "No events yet"}.</p>
