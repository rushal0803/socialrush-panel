      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
        <div className="space-y-6">
          <section className="rounded-3xl border border-white bg-white/90 p-5 shadow-sm sm:p-7">
            <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-blue-600">{order.platform || "Platform"}</p><h2 className="mt-2 text-2xl font-black text-[#112a5c]">{order.service_name || service?.name || "Growth service"}</h2></div><AdminStatus value={order.status} /></div>
            <a href={order.link} target="_blank" rel="noopener noreferrer" className="mt-5 block break-all rounded-2xl bg-blue-50 p-4 text-sm font-semibold text-blue-700 hover:underline">{order.link}</a>
          ...
          <section className="rounded-3xl border border-white bg-white/90 p-5 shadow-sm sm:p-7">
            <h2 className="text-lg font-black text-[#112a5c]">Fulfillment progress</h2>
            ...
            <div className="mt-5 h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-violet-500 via-blue-500 to-cyan-400" style={{ width: `${progress}%` }} /></div>
            <p className="mt-2 text-right text-xs font-bold text-blue-700">{order.progress_percent === null ? "Progress unavailable" : `${progress.toFixed(2)}% delivered`}</p>
        ...
        <aside className="space-y-6">
          <section className="rounded-3xl border border-white bg-white/90 p-5 shadow-sm">
            <h2 className="text-base font-black text-[#112a5c]">Customer</h2>
          ...
          <section className="rounded-3xl border border-white bg-white/90 p-5 shadow-sm">
            <h2 className="text-base font-black text-[#112a5c]">Payment & wallet deduction</h2>
          ...
          <section className="rounded-3xl border border-white bg-white/90 p-5 shadow-sm">
            <h2 className="text-base font-black text-[#112a5c]">Status timeline</h2>
            <div className="mt-5 space-y-4">{(history ?? []).map((item) => <div key={item.id} className="relative border-l-2 border-blue-100 pl-4"><span className="absolute -left-[5px] top-1 h-2 w-2 rounded-full bg-blue-500" /><AdminStatus value={item.status} /><p className="mt-1 text-[10px] text-slate-500">{new Date(item.created_at).toLocaleString("en-IN")}</p>{item.note ? <p className="mt-1 text-xs text-slate-600">{item.note}</p> : null}</div>)}{!history?.length ? <p className="text-xs text-slate-400">Timeline begins after the tracking migration is applied.</p> : null}</div>
          </section>
          <section className="rounded-3xl border border-white bg-white/90 p-5 shadow-sm">
            <h2 className="text-base font-black text-[#112a5c]">Operational details</h2>
