              ...
              <button
                ...
                className={`whitespace-nowrap rounded-xl px-6 py-3 text-sm font-bold transition ${
                  selectedPlatform === platform
                    ...
                    : "border border-white/10 bg-slate-950 text-slate-300 hover:border-cyan-500 hover:bg-slate-900"
                }`}
              >
                {platform}
              </button>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
            {platformPackages.map((pkg) => (
              <div
                key={pkg.packageId}
                className="group rounded-3xl border border-white/10 bg-[#0B1628]/95 p-6 shadow-2xl shadow-slate-950/40 backdrop-blur transition duration-300 hover:-translate-y-2 hover:border-cyan-500/40 hover:shadow-xl"
              >
                ...
                <h3 className="mt-2 text-2xl font-bold text-white">{pkg.title}</h3>
                {pkg.discountBadge && (
                ...
                <div className="mt-4 space-y-2 rounded-2xl border border-cyan-500/10 bg-slate-950/80 p-3">
                  <div className="flex items-center gap-2">
                    <MarketingIcon name="clock" className="h-4 w-4 text-cyan-300" />
                    <p className="text-xs font-bold text-cyan-200">Delivery: {pkg.deliveryTime}</p>
                  </div>
                  <p className="text-[11px] text-slate-300">{pkg.description}</p>
                </div>
