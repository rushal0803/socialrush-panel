...
export async function PUT(request: NextRequest) {
  ...
  if (!platform || !serviceCode || !target || !Number.isInteger(quantity) || quantity < 1 || platform.length > 40 || serviceCode.length > 100 || target.length > 2048) return NextResponse.json({ error: "Draft details are invalid." }, { status: 422 });
