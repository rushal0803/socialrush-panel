...
export async function POST(request: NextRequest) {
  ...
  const body = await request.json().catch(() => null) as { amount?: number; utr?: string; paymentReference?: string } | null;
  ...
  const { data: transaction, error } = await admin.from("transactions").insert({
    ...
    description: `UPI wallet top-up submitted for verification · Ref ${paymentReference} · UTR ${utr}`,
    metadata: { source: "manual_upi_wallet", payment_reference: paymentReference, utr },
