...
export default async function WalletPage() {
  ...
  const orders: WalletOrder[] = (orderRows ?? []).map((item) => ({ id: item.id, charge: Number(item.charge), status: item.status, created_at: item.created_at, serviceName: item.service_name || (item.services as unknown as { name?: string } | null)?.name || "Growth campaign" }));
