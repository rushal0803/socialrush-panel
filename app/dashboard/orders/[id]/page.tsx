...
export default function CustomerOrderDetailsPage() {
  ...
  return <main className="min-h-[calc(100vh-5rem)] overflow-x-clip bg-[radial-gradient(circle_at_top_left,rgba(255,122,0,.14),transparent_34%),#050505] px-4 pb-28 pt-5 text-white sm:px-6 lg:px-8">
    <div className="mx-auto max-w-6xl">
      ...
      <section className="mt-5 grid gap-3 rounded-3xl border border-white/10 bg-[#111111] p-4 sm:grid-cols-3 sm:p-5">{reorder ? <Link href={reorder} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#FF7A00] to-[#FFB000] px-4 text-sm font-black"><RefreshCw className="h-4 w-4" />Reorder</Link> : null}<Link href={supportHref} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-orange-400/25 bg-orange-500/10 px-4 text-sm font-bold text-orange-200"><ExternalLink className="h-4 w-4" />Get Support</Link>{order.refill_eligible ? <Link href={`${supportHref}&category=drop_or_refill`} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-emerald-400/25 bg-emerald-500/10 px-4 text-sm font-bold text-emerald-200">{order.refill_requested_at ? "View Refill Request" : "Request Refill"}</Link> : <p className="flex min-h-12 items-center justify-center rounded-xl border border-white/10 bg-white/[.03] px-4 text-center text-xs text-[#9CA3AF]">This order is not currently eligible for refill.</p>}</section>
