          ...
          <>
            <section ref={detailsRef} className="scroll-mt-40 mt-6 rounded-3xl border border-white/85 bg-white/72 p-5 shadow-[0_24px_54px_-34px_rgba(15,23,42,.5)] backdrop-blur-xl sm:p-6">
              ...
              <div className="mt-5 grid gap-5 lg:grid-cols-2">
                <label className="block text-xs font-black text-[#0B0B0F]"><span className="inline-flex items-center gap-2"><LinkIcon className="h-4 w-4" />{linkRule.label}</span><input value={targetLink} onChange={(event) => { setTargetLink(event.target.value); setError(""); }} placeholder={linkRule.placeholder} className="mt-2 min-h-13 w-full rounded-xl border border-[#FFF3E0] bg-white px-4 py-3.5 text-sm text-[#0B0B0F] outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-200/40" /><span className={`mt-2 block text-[11px] leading-5 ${linkError ? "text-rose-600" : "text-[#111827]"}`}>{linkError || linkRule.helper}</span></label>
                <label className="block text-xs font-black text-[#0B0B0F]"><span className="inline-flex items-center gap-2"><Hash className="h-4 w-4" />Quantity</span><input value={quantityInput} onChange={(event) => { setQuantityInput(cleanQuantity(event.target.value)); setError(""); }} inputMode="numeric" placeholder="Enter quantity" className="mt-2 min-h-13 w-full rounded-xl border border-[#FFF3E0] bg-white px-4 py-3.5 text-sm text-[#0B0B0F] outline-none transition focus:border-amber-400 focus:ring-4 focus:ring-amber-200/40" /><span className={`mt-2 block text-[11px] ${quantityError ? "text-rose-600" : "text-[#111827]"}`}>{quantityError || "Enter the quantity you want for this campaign."}</span></label>
              </div>
