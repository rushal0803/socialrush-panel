...
begin
 ...
 -- Re-grade from the newly calculated fit_score so thresholds never use a stale previous score.
 update crm_lead_candidates
 set fit_grade=case
   when compliance_status='blocked' or email_verification_status in ('invalid','bounced') then 'do_not_contact'
   when fit_score>=80 then 'high_fit'
   when fit_score>=60 then 'good_fit'
   when fit_score>=40 then 'research'
   else 'low_fit'
 end,
 research_status=case when fit_score>=40 and fit_score<60 then 'needed' else research_status end,
 updated_at=now()
 where qualification_status <> 'promoted';
