-- Keep the suppression list authoritative for every Autopilot score.
-- This only refreshes derived score/brief/health data; it does not send email
-- or modify lead, contact, customer, order, payment, or wallet source records.
...
begin
  ...
  left join lateral (
    select
      exists(
        ...
        where c.lead_id = l.id
          and (
            ...
            or exists (
              select 1 from crm_suppression_list s
      ...
      coalesce(
        (
          select 'Suppressed: ' || coalesce(nullif(trim(s.reason), ''), 'suppression list')
          from crm_lead_contacts c
          join crm_suppression_list s on lower(trim(s.email)) = lower(trim(c.email))
          where c.lead_id = l.id
        ...
        (
          select case
            when c.opted_out_at is not null then 'Unsubscribed'
            when c.verification_status = 'invalid' then 'Invalid or bounced email'
            else 'Suppressed or blocked contact'
          end
          from crm_lead_contacts c
          where c.lead_id = l.id
            and (c.opted_out_at is not null or c.verification_status = 'invalid' or c.compliance_status = 'blocked')
          limit 1
        ),
        'Do not contact'
      ) as reason
