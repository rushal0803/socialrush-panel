-- Keep internal Resend E2E records available for CRM/audit history, but out of business operations.
-- This migration remains derived-data only: it never sends email or changes source lead/contact records.

create or replace function public.crm_is_internal_operational_test_lead(lead_source_name text, lead_business_name text)
returns boolean language sql immutable as $$
  select coalesce(lead_source_name = 'Internal Resend E2E Test', false)
      or coalesce(lead_business_name = 'SocialRUSH Internal Test', false);
$$;
...
declare
  ...
  brief jsonb;
begin
  -- Scores are derived, so removing stale synthetic score rows preserves source/audit history.
  delete from crm_lead_scores s using crm_leads l
  where s.lead_id = l.id
    and public.crm_is_internal_operational_test_lead(l.source_name, l.business_name);
  ...
  left join lateral (
    select exists(
      select 1 from crm_lead_contacts c where c.lead_id = l.id and (
        c.opted_out_at is not null or c.verification_status = 'invalid' or c.compliance_status = 'blocked'
        or exists (select 1 from crm_suppression_list s where lower(trim(s.email)) = lower(trim(c.email)))
      )
    ) as blocked,
    coalesce(
      (select 'Suppressed: ' || coalesce(nullif(trim(s.reason), ''), 'suppression list') from crm_lead_contacts c join crm_suppression_list s on lower(trim(s.email)) = lower(trim(c.email)) where c.lead_id = l.id order by s.created_at desc limit 1),
      (select case when c.opted_out_at is not null then 'Unsubscribed' when c.verification_status = 'invalid' then 'Invalid or bounced email' else 'Suppressed or blocked contact' end from crm_lead_contacts c where c.lead_id = l.id and (c.opted_out_at is not null or c.verification_status = 'invalid' or c.compliance_status = 'blocked') limit 1),
      'Do not contact'
    ) as reason
  ) blockers on true
  left join lateral (
    select coalesce(sum(points), 0)::int as score,
      coalesce(jsonb_agg(jsonb_build_object('points', points, 'reason', reason) order by points desc) filter(where points <> 0), '[]'::jsonb) as reasons
    from (
  ...
  where not public.crm_is_internal_operational_test_lead(l.source_name, l.business_name)
  ...
  -- Webhook health deliberately includes internal E2E events: it measures technical processing health.
  ...
  select jsonb_build_object(
    'hot_leads', (select count(*) from crm_lead_scores s join crm_leads l on l.id = s.lead_id where s.grade = 'hot' and not public.crm_is_internal_operational_test_lead(l.source_name, l.business_name)),
    'interested_replies', (select count(*) from crm_inbound_messages m join crm_leads l on l.id = m.lead_id where m.received_at >= date_trunc('day', now() at time zone 'Asia/Kolkata') at time zone 'Asia/Kolkata' and m.classification in ('interested','meeting_request') and not public.crm_is_internal_operational_test_lead(l.source_name, l.business_name)),
    'replies_needing_review', (select count(*) from crm_inbound_messages m join crm_leads l on l.id = m.lead_id where m.received_at >= now() - interval '24 hours' and (m.needs_admin_review or m.classification = 'other') and not public.crm_is_internal_operational_test_lead(l.source_name, l.business_name)),
    'lead_followups_due', (select count(*) from crm_lead_reply_followups f join crm_leads l on l.id = f.lead_id where f.status = 'pending' and f.due_at <= now() and not public.crm_is_internal_operational_test_lead(l.source_name, l.business_name)),
