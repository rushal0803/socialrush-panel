...
begin
 select jsonb_build_object('high_fit_prospects',count(*) filter(where fit_grade='high_fit'),'new_qualified_candidates',count(*) filter(where qualification_status in ('qualified','ready') and created_at >= now()-interval '24 hours'),'research_needed',count(*) filter(where research_status='needed'),'duplicate_candidates',count(*) filter(where duplicate_lead_id is not null and not duplicate_override),'suppressed_candidates',count(*) filter(where qualification_status='blocked')) into extra from crm_lead_candidates;
